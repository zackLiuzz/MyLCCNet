...
@ex.config
def config():
    checkpoints = './checkpoints_itr5/'
    dataset = 'kitti/odom_itr5' # 'kitti/raw'
    data_folder = '/root/paddlejob/workspace/env_run/dataset/dataset/'
    ...
    optimizer = 'adam'
    resume = True
    weights = './LCCNet/pretrained/my_itr4.tar'
    #resume = False
    #weights = None
    rescale_rot = 1.0
    rescale_transl = 2.0
...
def main(_config, _run, seed):
    ...
    if _config['weights'] is not None:
        ...
        print(f"Loaded weights from {_config['weights']}")
    ...
    for epoch in range(starting_epoch, _config['epochs'] + 1):
        ...
        for batch_idx, sample in enumerate(TrainImgLoader):
            ...
            print("gonna run train for batch: %d, idx: %d" % (batch_idx, idx))
